...
async function genDictionary(seed, selectedCharset) {
	...
	function makeChar(size, char) {
		let newChar = "1" + randBinary(seed + charset[char].join("_").replaceAll(" ", "_") + "_" + size, size) + "1";
	...
	charkeys.forEach((char, i) => {
		let s = 2
		let newChar = makeChar(s, char);
		while (Object.values(chars).includes(newChar)) {
			newChar = makeChar(s, char);
			s+=2;
		}
		if (i % 20 == 0) {
			//progress bar
			loadingBarSet(i / charkeys.length * 100, "| " + i + "/" + charkeys.length + " | " + char);
		}
		chars[char] = newChar;
	})
	const end = Date.now();
...
async function encode(toEncode, key) {
	//step 1: get dictionary
	const dict = await getDictionary(key, "chars");
...
async function decode(toDecode, key) {
	//step 1: get dictionary

	const dict = await getDictionary(key, "chars", true);
...
async function test() {
	...
	console.log("=============");
	console.log(randBinary(123, 500))
}
genDictionary(123, "chars").then(() => {
test();
})
